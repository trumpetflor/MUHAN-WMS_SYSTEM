...
<script type="text/javascript">
	...
	function load_list() {
		$.ajax({
			type: "GET",
			url: "../Product/GroupBottomSelectListJson?keyword=" + $("#keyword").val() + "&searchType=" + $("#searchType").val() , 
			dataType: "json"
		})
		.done(function(Group_bottom_table) { // 요청 성공 시
			let result ="";
			for(let groupList of Group_bottom_table) {
				result += "<tr>"
							+ "<td onclick='fn_selectGroup(\"" + groupList.product_group_bottom_cd + "\",\""+groupList.product_group_bottom_name+ "\")'>"+ groupList.product_group_bottom_cd + "</td>"
							+ "<td onclick='fn_selectGroup(\"" + groupList.product_group_bottom_cd + "\",\""+groupList.product_group_bottom_name+ "\")'>"+ groupList.product_group_bottom_name + "</td>"
							+ "</tr>";
				
			}
			$("#Group_bottom_table > tbody").html(result);
		})
		.fail(function() {
			$("#Group_bottom_table").append("요청 실패!!");
		});
	}

	function fn_selectGroup(product_group_bottom_cd, product_group_bottom_name) {
		opener.fn_selectGroup(product_group_bottom_cd, product_group_bottom_name);
		window.close();
   ...
   <div class="animated fadeIn">
	<section id="searchSection" class="m-0 d-flex justify-content-end">
 					...
 					<select name="searchType" id="searchType" class="rounded-1 btn-sm p-1">
						<option value="product_group_bottom_cd">품목 그룹 코드</option>
						<option value="product_group_bottom_name">품목 그룹명</option>
					</select>	
...
<div id="modal_container" class="modal">
	<form action="ProdGroupBottomNameInsertPro" method="post" >
		<div class=" m-3 border border-light border-top-0 rounded-2 border border-1"> 
			<div class="p-2 bg-light text-black well rounded-2" >그룹 신규 등록</div>
			<table class=" table">
				<tr>
					<th>그룹명</th>
					<td><input type="text" class="form-control" name="product_group_bottom_name"  required="required"></td>
				</tr>
			</table>
		</div>
