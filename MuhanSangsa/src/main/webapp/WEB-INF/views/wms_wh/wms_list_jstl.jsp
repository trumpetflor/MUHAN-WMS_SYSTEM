		...
		<div class="content d-flex">
			<div>
			    <ul style=" width: 410px;">
			    	...
			    	<c:set var="whName">${wh.wh_name }</c:set> <!-- 창고명 저장 -->
			        <li class="menu" style="">
			            <a><img src="./resources/images/right-arrow.png" width="10px" />&nbsp;<i>${whName }</i> 
			             <input type="button" value="+" id="addColumn" style="float: right;"></a>
				    	<c:forEach var="whWha" items="${whWhaList[whName] }" varStatus="whWha_idx"> <!-- 창고 구역 반복 -->
				    	<c:set var="whArea">${whWha.wh_area }</c:set> <!-- 창고 구역명 저장 -->
			            <ul class="hide">
			                <li class="w1"><a><img src="./resources/images/right-arrow.png" width="10px" />&nbsp;${whArea }
			                <input type="button" value="+" style="float: right;"><input type="button" value="-" style="float: right;"></a></li>
						    	<c:forEach var="whWhla" items="${whWhlaList[whArea] }" varStatus="whWhla_idx"> <!-- 창고 구역 내 위치 반복 -->
						    	<c:set var="whLocInArea" >${whWhla.wh_loc_in_area }</c:set> <!-- 창고 구역 내 위치명 저장 -->
			                	<ul class="hide">
			                 		<li class="w2">${whLocInArea }<a>&nbsp;
			                 		<input type="button" value="+" style="float: right;"><input type="button" value="-" style="float: right;"></a></li>
		                 		</ul>
<%-- 						        창고 구역 위치 명 : ${whLocInArea } / 창고 구역 인덱스 : ${whWhla_idx.index } --%>
		                 		</c:forEach>
			            </ul>
<%-- 				        창고 구역 명 : ${whArea } / 창고 구역 인덱스 : ${whWha_idx.index } --%>
			            </c:forEach>
				        </li>
<%-- 				        <c:out value="${whName }"></c:out> --%>
			        </c:forEach>
			    </ul>
			</div>
