...
<script type="text/javascript">
	$(function() {
		console.log("확인"+ ${in_schedule_cd});
		load_list(); // 게시물 목록 조회 함수 호출
		
	});
	// 거래처 목록 조회 함수
	function load_list() {
		console.log(${in_schedule_cd});
		$.ajax({
			type: "GET",
			url:"inProQtySearch?in_schedule_cd=" + ${in_schedule_cd} , 
			dataType: "json"
		})
		.done(function(ProQtyList) { // 요청 성공 시
			let result ="";
			for(let ProQty of ProQtyList) {
				result += "<tr>"
							+ "<td id="+ProQty.product_cd+">" + ProQty.product_cd + "</td>"
							+ "<td id="+ProQty.product_name+">" + ProQty.product_name + "</td>"
							+ "<td id="+ProQty.in_schedule_qty+">" + ProQty.in_schedule_qty + "</td>"
							+ "<td id="+ProQty.no_in_qty+">" + ProQty.no_in_qty + "</td>"
							+ "</tr>";
